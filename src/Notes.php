...
class Notes extends XML {
	...
	public function Query(\mysqli &$db, int $uid, int $cmd, $parm = '') {
		...
		if (!$this->_ids) {
			...
			$obj = $db->query('SELECT `id` FROM `myapp_notes` WHERE `user` = "'.$uid.'"');
		...
		case DataStore::DEL:
			...
			// delete record
			$db->query('DELETE FROM `myapp_notes` WHERE `id` = "'.
		...
		case DataStore::RGID:
			...
			$obj = $db->query('SELECT * FROM `myapp_notes` WHERE `id` = "'.
